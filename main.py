...
START   = 1000.0  # Starting paper balance reference
...
STD_PCT    = 0.20   # Standard: 20% of cash
HUNT_PCT   = 0.30   # Hunter single signal: 30%
HUNT2_PCT  = 0.40   # Hunter double-confirmed: 40%
STD_ATR    = 1.5    # Standard ATR stop multiplier
HUNT_ATR   = 2.0    # Hunter ATR stop multiplier (wider)
...
S = {
    ...
    "last_ai":   0,    # timestamp of last AI decision
    "cash":      0.0,  # from Alpaca account
    "status":    "connecting",
    "cycles":    0,
}
...
# ── PRICE FEED (CoinGecko — free, no key) ────────────────────────────
def get_prices():
    try:
        r = requests.get(
...
def buy(sym, notional):
    """Place a notional market buy order on Alpaca"""
    try:
        payload = {
            "symbol":        sym,
...
def sell_all(sym):
    """Close entire position on Alpaca"""
    try:
...
def check_stops():
    ...
    for label in list(S["positions"].keys()):
        ...
        # Raise trailing stop (never lower it)
        new_stop = cur - atr * mult
        pos["stop"] = max(pos["stop"], new_stop)

        # Break-even: once up 5% (std) or 8% (hunter), stop moves to entry
        be = 1.08 if pos["mode"] == "hunter" else 1.05
        if cur >= pos["entry"] * be and not pos.get("be"):
            pos["stop"] = max(pos["stop"], pos["entry"])
            pos["be"] = True
            log(f"⚓ Break-even set {label} — worst case now: $0 loss", "info")

        # Scale out: sell 30% at +20%
        if cur >= pos["entry"] * 1.20 and not pos.get("so"):
            partial = pos["qty"] * 0.30
            asset = next(a for a in ASSETS if a["label"] == label)
            sell_all(asset["sym"])  # simplified: close full then reopen 70% not implemented in paper
            pos["so"] = True
            profit = (cur - pos["entry"]) * partial
...
def ai_decide(cash):
    now = time.time()
    if now - S["last_ai"] < 180:   # 3 minute cooldown
        return
    if not S["prices"]:
        return
    S["last_ai"] = now
    ...
    try:
        ...
        ) or "None — looking for entry"

        prompt = f"""Crypto trading agent. Paper mode: {PAPER}

ACCOUNT: Cash ${cash:.2f} | Open positions: {len(S['positions'])}/2 max
...
STRATEGY RULES:
- BUY only when 24h momentum >3% and asset not already held
- HUNTER mode if 24h change >8% OR 2+ strong signals confirm
- Standard position: 20% of cash | Hunter: 30% (40% if double-confirmed)
- Max 2 open positions at any time
- Be patient and selective — only high-conviction setups qualify

Respond ONLY with this exact JSON (no markdown, no extra text):
{{"action":"BUY or HOLD","asset":"BTC or ETH or SOL or null","mode":"standard or hunter or null","confidence":7,"reason":"8 words max","hunter_signal":"SHORT_SQUEEZE or MOMENTUM or GAP or LOW_FLOAT or NEWS or null","double_confirmed":false}}"""

        msg = client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=120,
            system="You are a trading agent. Reply only with valid JSON. No markdown.",
            ...
            if asset and label not in S["positions"] and label in S["prices"]:
                ...
                if notional > 10:
                    ...
                    if order:
                        ...
                        mode_label = "🎯 HUNTER" if ih else "📈 STANDARD"
                        log(f"{mode_label} BUY {label} @${price:,.2f} | {sz} | stop ${stop:.0f} | {dec.get('reason','')}", "hunter" if ih else "buy")

        else:
            log(f"💭 HOLD — {dec.get('reason','no clear setup')} (confidence {dec.get('confidence',7)}/10)", "info")
...
def trading_loop():
    ...
    log(f"📡 Connected to Alpaca {'paper' if PAPER else 'live'} API")
    time.sleep(3)
    while True:
        try:
            prices = get_prices()
...
.st.fired{background:#00C89620;color:#00C896}
</style></head>
<body><div id="app"><div style="text-align:center;padding:60px 20px;color:#5A6478;font-size:11px">Loading agent...</div></div>
<script>
let tab='market',data={};
...
async function load(){
  try{const r=await fetch('/api');data=await r.json();draw();}catch(e){console.error(e);}
}

function draw(){
  if(!data.prices)return;
  const total=(data.cash||0)+(data.pos_value||0);
  const pnl=total-1000,pnlP=pnl/1000*100;
  const wins=(data.trades||[]).filter(t=>t.pnl>0).length;
  const wr=data.trades?.length?Math.round(wins/data.trades.length*100):0;
  const pc=pnl>=0?'g':'r';
  const live=data.status==='live';
  const cc=live?'#00C896':data.status==='error'?'#EF4444':'#F5A623';
  ...
  <div class="head"><div class="brand">YISROEL & YAKOV</div>
  <div class="stats">
  ...
  <div class="tabs">
    ${['market','trades','hunter','log'].map(t=>`<button class="tab${tab===t?' on':''}" onclick="go('${t}')">${t.toUpperCase()}</button>`).join('')}
  </div><div class="body">`;

  if(tab==='market'){
    h+=`<div class="lbl" style="margin-top:0">LIVE PRICES</div>`;
    ['BTC','ETH','SOL'].forEach(id=>{
      const pr=data.prices?.[id],pos=data.positions?.[id];
      if(!pr)return;
      const mc=pr.chg24h>=0?'g':'r';
      h+=`<div class="card" style="${pos?'border-color:#FF6B3560':''}">
        <div class="ch">
          ...
          <span class="sv ${mc}" style="font-size:11px">${p(pr.chg24h||0)}</span>
        </div>
      ...
      if(pos){h+=`<div class="hr"></div>
        ...
        <div class="kv"><span class="k">ATR trailing stop</span><span class="v r">$${pos.stop?.toFixed(2)}</span></div>
        <div class="kv"><span class="k">Paper P&L</span><span class="v ${pos.pnl>=0?'g':'r'}">${pos.pnl>=0?'+':''}${$(pos.pnl||0)}</span></div>`;}
      h+=`</div>`;
    });
    h+=`<div class="card">
      <div class="kv"><span class="k">Cash (Alpaca paper)</span><span class="v">${$(data.cash||0)}</span></div>
      <div class="kv"><span class="k">In positions</span><span class="v">${$(data.pos_value||0)}</span></div>
      <div class="kv"><span class="k">Mode</span><span class="v ${data.paper?'g':'o'}">${data.paper?'● PAPER TRADING':'● LIVE TRADING'}</span></div>
    </div>`;
  }
  ...
  if(tab==='trades'){
    ...
    if(op.length){h+=`<div class="lbl" style="margin-top:0">OPEN POSITIONS</div>`;
      op.forEach(([lbl,pos])=>{
        h+=`<div class="card" style="border-color:${pos.mode==='hunter'?'#FF6B3560':'#1C2333'}">
          <div class="ch">
            <div style="display:flex;gap:8px;align-items:center"><span style="font-size:14px;font-weight:bold">${lbl}</span>
            ...
            <span class="sv ${pos.pnl>=0?'g':'r'}" style="font-size:11px">${pos.pnl>=0?'+':''}${$(pos.pnl||0)}</span>
          </div>
          <div class="kv"><span class="k">Entry</span><span class="v">$${pos.entry?.toLocaleString('en-US',{maximumFractionDigits:2})}</span></div>
    ...
    else data.trades.slice(0,15).forEach(t=>{
      h+=`<div class="card">
        <div class="ch">
          <div style="display:flex;gap:8px;align-items:center"><span style="font-size:12px;font-weight:bold">${t.asset}</span>
          ...
          <span class="sv ${t.pnl>=0?'g':'r'}" style="font-size:11px">${t.pnl>=0?'+':''}${$(t.pnl)}</span>
        </div>
  ...
  if(tab==='hunter'){
    h+=`<div class="card" style="border-color:#FF6B3550">
      <div class="ch"><span style="font-size:13px;font-weight:bold;color:#FF6B35">🎯 Hunter Mode</span><span style="font-size:8px;color:#00C896;letter-spacing:1px">● ACTIVE</span></div>
      <div style="font-size:9px;color:#5A6478;line-height:1.7;margin-bottom:10px">Explosive 50–300%+ move scanner. ATR stop protects every trade.</div>
      ${[['Single signal','30% position','#FF6B35'],['2+ confirmed','40% position','#EF4444'],['ATR stop','2x ATR wider','#5A6478'],['Break-even','+8% trigger','#5A6478'],['Scale out','20% at +30%','#5A6478'],['Options','Top 1-2 trades/month','#F5A623']].map(([k,v,c])=>`<div class="kv"><span class="k">${k}</span><span style="font-size:9px;font-weight:bold;color:${c}">${v}</span></div>`).join('')}
    </div>
    <div class="lbl">SIGNAL SCANNERS</div>
    <div class="card" style="padding:0 13px">
      ${[['01','Short Squeeze','Short float >20% + days-to-cover >5'],['02','Pre-Market Gap','Gap >4% before open with volume'],['03','Crypto Funding','Extreme negative funding rates'],['04','Low Float Spike','Float <10M + 3% in 2 mins'],['05','News Catalyst','FDA / earnings / listing / Reddit']].map(([n,nm,d2])=>`<div class="sig"><div style="display:flex;justify-content:space-between;align-items:center"><span class="sn">${n} · ${nm}</span><span class="st">WATCHING</span></div><div class="sd">${d2}</div></div>`).join('')}
    </div>`;
  }

  if(tab==='log'){
    h+=`<div class="lbl" style="margin-top:0">AGENT ACTIVITY LOG</div>`;
    if(!data.log?.length)h+=`<div class="empty">Agent starting up...</div>`;
